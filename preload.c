...
{
    int fd = -1, socket_fd=-1,socket_fd1 = -1,nbytes;
    char buffer[256];
    ...
    write(socket_fd,buffer,nbytes);
    	...
    	printf("Modified LibC: Sending open() request : %s\n", buffer);
    	write(socket_fd,buffer,nbytes);
    ...
    printf("LibC : Sending the function name : %s %d\n", buffer,capid);
    write(socket_fd,buffer,nbytes);
        //printf("%d\n", socket_fd);
        nbytes = read(socket_fd, buffer, 256);
        buffer[nbytes] = 0;
...
/*
  ...
  int *list;
        list = pidof("creat_test");

    for (i=0; list[i] != -1; i++)

          {
            printf("pid of creat is %d ", list[i]);
          }
          free (list);
          if (list[0] != -1)
          {
 ...
 /*---------------------------------------------------------------------------------------------------------*/
          ...
          cap_from_name("cap_audit_control", &cap_list);
          printf("Modified LibC: POSIX Capability flag set : %-20s %d\n", "cap_audit_control", cap_list);
        	printf("flags: ");
  ...
  k = 1;
          if(k)
          {
              printf("Modified LibC: Capability mode... Diverting the request to CapD via UDS...\n\n");
              client_creat(path,mode);
              return daemonfd;
          }
          else
          {
              printf("Modified LibC: Normal mode..Calling the real creat\n");
...
void client_creat(const char *path,mode_t mode)
{
	...
	printf("error");
	 ...
	 /*cap_create request*/
   	...
   	printf("Capability Object Creation\n");
    	nbytes=snprintf(buffer,256,"%s","cap_create");
    	printf("Modified LibC: Sending capability create request : %s\n", buffer);
    	write(socket_fd,buffer,nbytes);
    ...
    //  printf("%s\n", buffer);
        ...
        printf("Send creat request\n");
    	nbytes=snprintf(buffer,256,"%s","creat");
    	printf("Modified LibC: Sending creat request : %s\n", buffer);
    	write(socket_fd,buffer,nbytes);
        ...
        nbytes=snprintf(buffer,256,"%s",pathname);
        printf("Modified LibC: Sending resource specification from xml file : %s\n", buffer);
       // printf("%d\n",nbytes);
       // printf("hellooooooo");
       write(socket_fd,buffer,nbytes);
...
//memset(&buffer,'0',sizeof(buffer));
  ...
  //  	printf("%s\n", buffer);
    	...
    	nbytes=snprintf(buffer,256,"%s",perm);
    	printf("Modified LibC: Sending operation specification from xml file : %s\n\n", buffer);
    	write(socket_fd,buffer,nbytes);
  //  	printf("%s\n", buffer);
...
nbytes=snprintf(buffer,256,"%s %s","creat",capoid);
   printf("Invocation of CREAT\n");
    printf("Modified LibC: Sending the function name and capoid : %s \n", buffer);
    write(socket_fd,buffer,nbytes);
...
*/
nbytes=snprintf(buffer,256,"%s",path);
   printf("Modified LibC: Sending file path : %s\n", buffer);
   write(socket_fd,buffer,nbytes);
...
//
nbytes=snprintf(buffer,20,"%s",perm);
   printf("Modified LibC: Sending file mode : %s\n", perm);
   write(socket_fd, buffer, nbytes);
     ...
     nbytes = read(socket_fd, buffer, 256);
       ...
       int fdrecv[5];
      memset(&fdrecv,'0',sizeof(fdrecv));
      // printf("hey iam socket :%d\n",socket_fd);
       //printf("hey iam  fdrecv:%d\n",fdrecv[0]);
...
/*end of client creat() function code*/
